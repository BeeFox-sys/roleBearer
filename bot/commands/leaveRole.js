...
module.exports = {
	...
	async execute(message, args) {
        ...
        let role = message.guild.roles.cache.filter(role => guild.roles.has(role.id)).find(role => role.name.toLowerCase().replace(/[^\w\s]/gi,"") == roleName.toLowerCase().replace(/[^\w\s]/gi,""))
